...
baseForOwn(LazyWrapper.prototype, function(func$$1, methodName) {
  ...
  lodash.prototype[methodName] = function() {
    var value$$1 = this.__wrapped__,
        ...
        iteratee$$1 = args[0],
        useLazy = isLazy || isArray(value$$1);
...
arrayEach(['pop', 'push', 'shift', 'sort', 'splice', 'unshift'], function(methodName) {
  ...
  lodash.prototype[methodName] = function() {
    ...
    if (retUnwrapped && !this.__chain__) {
      ...
      return func$$1.apply(isArray(value$$1) ? value$$1 : [], args);
    }
...
var API_ROOT = 'http://frontend-test.pingbull.com/pages/endid13@gmail.com/';
...
var CommentsModel = function () {
    function CommentsModel(root_el) {
        classCallCheck(this, CommentsModel);
        ...
        bindAll(this, ['setState', 'renderComments',
        // 'appendComment',
        'fetchComments', 'sendComment', 'removeComment', 'editComment', 'afterRender', 'beforeRemove', '_handleDelete', '_handleEdit', '_handleReply']);
    }

    createClass(CommentsModel, [{
        key: 'setState',
        ...
        value: function renderComments() {
            ...
            forEach(this.state.comments, function (comment) {
                var comment_datetime = new Date(comment.updated_at ? comment.updated_at : comment.created_at);
                var comment_date = moment(comment_datetime).format('YYYY-MM-DD');
                var comment_time = moment(comment_datetime).format('HH:mm');

                var comment_el = document.createElement('article');
                comment_el.className = 'comments-list__comment comment';
                comment_el.setAttribute('data-id', comment.id);

                var owner_buttons = comment.author.id === _this.state.owner_id ? '\n<div class="comment-footer__section">\n    <button type="button" class="comment-footer__btn btn-edit" data-id="' + comment.id + '" data-parent="' + null + '">\n        <i class="fa fa-pencil-square-o" aria-hidden="true"></i> Edit\n    </button>\n</div>\n<div class="comment-footer__section">\n    <button type="button" class="comment-footer__btn btn-delete" data-id="' + comment.id + '" data-parent="' + null + '">\n        <i class="fa fa-times" aria-hidden="true"></i> Delete\n    </button>\n</div>\n            ' : '';
                ...
                _this.root_el.appendChild(comment_el);

                if (comment.children && comment.children.length) {
                    var nested_root = document.createElement('div');
                    nested_root.className = 'comment__nested-comments';

                    _this.root_el.querySelector('[data-id="' + comment.id + '"] .comment__wrapper').appendChild(nested_root);

                    forEach(comment.children, function (child) {
                        var comment_datetime = new Date(child.updated_at ? child.updated_at : child.created_at);
                        var comment_date = moment(comment_datetime).format('YYYY-MM-DD');
                        var comment_time = moment(comment_datetime).format('HH:mm');
        ...
        value: function afterRender() {
            ...
            forEach(delete_btns, function (btn) {
                btn.addEventListener('click', _this2._handleDelete);
            });

            var edit_btns = document.querySelectorAll('.btn-edit');
            forEach(edit_btns, function (btn) {
                btn.addEventListener('click', _this2._handleEdit);
            });

            var reply_btns = document.querySelectorAll('.btn-reply');
            forEach(reply_btns, function (btn) {
                btn.addEventListener('click', _this2._handleReply);
            });
        }
    }, {
        key: 'beforeRemove',
        value: function beforeRemove() {
            ...
            var delete_btns = document.querySelectorAll('.btn-delete');
            forEach(delete_btns, function (btn) {
                btn.removeEventListener('click', _this3._handleDelete);
            });

            var edit_btns = document.querySelectorAll('.btn-edit');
            forEach(edit_btns, function (btn) {
                btn.removeEventListener('click', _this3._handleEdit);
            });

            var reply_btns = document.querySelectorAll('.btn-reply');
            forEach(reply_btns, function (btn) {
                btn.removeEventListener('click', _this3._handleReply);
            });
        }
    }, {
        key: '_handleDelete',
        ...
        key: '_handleEdit',
        value: function _handleEdit(e) {
            // this.editComment(e.target.getAttribute('data-id'), e.target.getAttribute('data-parent'));
        }
    }, {
        key: '_handleReply',
        value: function _handleReply(e) {
            var _this4 = this;

            var comment_wrapper = e.target.parentNode.parentNode.parentNode;
            var nested = comment_wrapper.querySelector('.comment__nested-comments');

            var reply_to_name = e.target.getAttribute('data-name');
            var reply_to_id = e.target.getAttribute('data-id');
            ...
            comment_wrapper.querySelector('.reply-info__cancel').addEventListener('click', function () {
                _this4.removeReply();
            });

            comment_wrapper.querySelector('.comment-reply__form').addEventListener('submit', function (e) {
                e.preventDefault();
                var textarea = e.target.querySelector('.textarea');

                if (textarea.value) {
                    _this4.sendComment(textarea.value, e.target.getAttribute('data-id'));
                    // textarea.value = '';
                    _this4.removeReply();
                }
            });
        ...
        value: function removeReply() {
            forEach(document.querySelectorAll('.comment-reply'), function (form) {
                form.remove();
            });
        }
    }, {
        ...
        value: function fetchComments() {
            ...
            if (this.state.hasMore) {
                var query = API_ROOT + '/comments?offset=' + this.state.offset + '&count=' + count;
                ...
                fetch(query).then(function (response) {
                    ...
                    _this5.setState({
                        comments: response,
                        offset: _this5.state.offset + count,
                        hasMore: response.length >= 5
                    });

                    _this5.renderComments();
        ...
        key: 'sendComment',
        value: function sendComment(content) {
            var _this6 = this;

            var parent = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;

            var query = API_ROOT + '/comments';
            ...
            fetch(query, {
                ...
                if (parent && parent != 'null') {
                    var idx = findIndex(_this6.state.comments, function (comment) {
                        return comment.id == parent;
                    });
                    _this6.state.comments[idx].children.unshift(response);
                } else {
                    var new_length = _this6.state.comments.unshift(response);

                    _this6.setState({
                        offset: _this6.state.offset + 1
                    });
                }

                _this6.renderComments();
            }).catch(function (e) {
                console.error(e);
            });
        }
        ...
        key: 'removeComment',
        value: function removeComment(id, parent) {
            var _this7 = this;

            var query = API_ROOT + '/comments/' + id;

            fetch(query, {
                method: 'DELETE'
            }).then(function () {
                if (parent && parent != 'null') {
                    var idx = findIndex(_this7.state.comments, function (comment) {
                        return comment.id == parent;
                    });
                    _this7.state.comments[idx].children = remove(_this7.state.comments[idx].children, function (comment) {
                        return comment.id != id;
                    });
                } else {
                    _this7.state.comments = remove(_this7.state.comments, function (comment) {
                        return comment.id != id;
                    });
        ...
        key: 'editComment',
        value: function editComment(id, parent) {
            var query = API_ROOT + '/comments/' + id;
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var comments = new CommentsModel('.comments-list');

    // Загрузка коментариев
    comments.fetchComments(5);

    var load_more_btn = document.querySelector('.load-more__btn');
    load_more_btn.addEventListener('click', function () {
        comments.fetchComments(5);
    });

    var leave_comment_form = document.querySelector('.leave-comment__form');
    leave_comment_form.addEventListener('submit', function (e) {
        e.preventDefault();
        var textarea = e.target.querySelector('.textarea');
